  ...
  Future<void> _startNewTrip() async {
    ...
    try {
      ...
      if (conductorDoc.docs.isNotEmpty) {
          ...
          if (!dailyTripDoc.exists) {
            await FirebaseFirestore.instance
              ...
              'trip1': {
                ...
                'direction': routeDirections.firstWhere(
                    (r) => r['collection'] == selectedPlaceCollection)['label'],
                'placeCollection': selectedPlaceCollection,
                'isComplete': false,
                'tripId': tripId,
              },
            ...
            if (currentTrip % 2 == 1 &&
                dailyTripData?['trip$currentTrip']?['isComplete'] == true) {
              final currentTripPlaceCollection =
                  dailyTripData?['trip$currentTrip']?['placeCollection'] ??
                      'Place';
              final oppositePlaceCollection =
                  currentTripPlaceCollection == 'Place' ? 'Place 2' : 'Place';
            ...
            } else if (dailyTripData?['trip$currentTrip']?['isComplete'] ==
                false) {
              final currentTripPlaceCollection =
                  dailyTripData?['trip$currentTrip']?['placeCollection'] ??
                      'Place';
              ...
              await FirebaseFirestore.instance
                ...
                'trip$nextTripNumber': {
                  'startTime': Timestamp.fromDate(now),
                  'direction': routeDirections.firstWhere((r) =>
                      r['collection'] == selectedPlaceCollection)['label'],
                  'placeCollection': selectedPlaceCollection,
                  'isComplete': false,
                  'tripId': tripId,
                },
        ...
        final currentTripNumber = dailyTripDoc.exists
            ? (dailyTripDoc.data()?['currentTrip'] ?? 1)
            : 1;
        ...
        await FirebaseFirestore.instance
          ...
          'activeTrip': {
            ...
            'startTime': Timestamp.fromDate(now),
            'tripId': tripId,
            'isReturnTrip': currentTripNumber == 2,
          },
          'passengerCount': 0,
        });
        ...
        setState(() {
          isTripActive = true;
          currentTripDirection = routeDirections.firstWhere(
              (r) => r['collection'] == selectedPlaceCollection)['label'];
          tripStartTime = now;
              ...
              child: Column(
                ...
                children: [
                  ...
                  ...routeDirections.map((direction) {
                    final isSelected =
                        direction['collection'] == selectedPlaceCollection;
                  ...
                  if (isTripActive) ...[
                    ...
                    StreamBuilder<QuerySnapshot>(
                      ...
                      builder: (context, snapshot) {
                        ...
                        final isLocationTrackingActive =
                            conductorData['isOnline'] ?? false;
                        ...
                        return Column(
                          children: [
                            Container(
                              ...
                              decoration: BoxDecoration(
                                color: isLocationTrackingActive
                                    ? Colors.green[50]
                                    : Colors.red[50],
                                borderRadius: BorderRadius.circular(12),
                                border: Border.all(
                            ...
                            SizedBox(
                              ...
                              child: ElevatedButton(
                                ...
                                onPressed: isLocationTrackingActive
                                    ? _startNewTrip
                                    : null,
                                child: Text(
                                  'Start Trip',
...
Future<void> storePreTicketToFirestore(Map<String, dynamic> data) async {
  ...
  print('🔍 Parsed quantity: $quantity from raw: $rawQuantity');
...
Future<void> _processPreBooking(Map<String, dynamic> data, User user,
  ...
  // Update the original pre-booking document
  ...
  // Store in conductor's preBookings collection (for trip management)
  await FirebaseFirestore.instance
      .collection('conductors')
      .doc(conductorDocId)
      .collection('preBookings')
