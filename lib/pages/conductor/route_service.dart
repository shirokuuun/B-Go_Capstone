...
class RouteService {
  static Future<String> fetchRoutePlaceName(String route) async {
    ...
    if (doc.exists) {
      ...
      } else if (route.trim() == 'Mataas Na Kahoy Palengke' &&
  ...
  static Future<List<Map<String, dynamic>>> fetchPlaces(String route,
      {String? placeCollection}) async {
    try {
      // Get the Firestore instance
      FirebaseFirestore firestore = FirebaseFirestore.instance;

      // Reference to the Destinations collection
      CollectionReference destinationsRef =
      ...
      if (querySnapshot.docs.length > 10) {
        print(
            '🔍 RouteService: Large dataset detected (${querySnapshot.docs.length} docs) - reducing debug output');
      } else {
      ...
      List<Map<String, dynamic>> places = querySnapshot.docs.map((doc) {
        ...
        return {
          'name': data['Name']?.toString() ??
              doc.id, // Use 'Name' field or doc.id as fallback
          'km': distance,
  ...
  static Future<String> saveTrip({
    ...
    List<String> formattedFares =
        discountedFares.map((f) => f.toStringAsFixed(2)).toList();
    ...
    final conductorId =
        await RouteService.getConductorDocIdFromUid(user?.uid ?? '');
    if (conductorId == null) {
    ...
    try {
      remittanceSummary = await RemittanceService.calculateDailyRemittance(
          conductorId, formattedDate);
      await RemittanceService.saveRemittanceSummary(
          conductorId, formattedDate, remittanceSummary);
        ...
        try {
          Map<String, dynamic> remittanceSummary =
              await RemittanceService.calculateDailyRemittance(
                  conductorId, date);
          await RemittanceService.saveRemittanceSummary(
              conductorId, date, remittanceSummary);
  ...
  static Future<List<Map<String, dynamic>>> getManualTickets(
    ...
    try {
      final ticketsSnapshot = await FirebaseFirestore.instance
          ...
          .collection('tickets')
          .where('ticketType',
              isEqualTo: 'Manual') // ✅ CHANGE from 'manual' to 'Manual'
          .get();
...
// Fetch available trip dates for a conductor
  static Future<List<String>> fetchAvailableTripDates(
    ...
    try {
      ...
      try {
        ...
        if (remittanceDoc.exists) {
          ...
          final remittanceTicketsSnapshot =
              await remittanceDoc.reference.collection('tickets').get();

          if (remittanceTicketsSnapshot.docs.isNotEmpty) {
            print(
                '🔍 Found tickets in remittance: ${remittanceTicketsSnapshot.docs.length}');
...
//  Delete a ticket
  ...
  Future<void> sendSOS({
    ...
    await FirebaseFirestore.instance.runTransaction((transaction) async {
      ...
      final paddedCount =
          newCount.toString().padLeft(3, '0'); // sos_001, sos_002, etc.
      final newDocId = 'sos_$paddedCount';
      final newDocRef =
          FirebaseFirestore.instance.collection('sosRequests').doc(newDocId);
...
// Find latest SOS by route and update its status
  ...
  Future<void> cancelSOS(String docId) async {
    final docRef =
        FirebaseFirestore.instance.collection('sosRequests').doc(docId);
    await docRef.update({
      'status': 'Cancelled',
      'isActive': false,
    });
